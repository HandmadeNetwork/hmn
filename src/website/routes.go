...
func NewWebsiteRoutes(conn *pgxpool.Pool, perfCollector *perf.PerfCollector) http.Handler {
	...
	hmnOnly.GET(hmnurl.RegexTicketsAdminEventCSVDownload, adminsOnly(TicketsAdminEventCSVDownload))
	hmnOnly.POST(hmnurl.RegexTicketsAdminReserve, adminsOnly(TicketsAdminReserve))
	hmnOnly.GET(hmnurl.RegexTicketsAdminScan, adminsOnly(TicketsAdminScan))
	hmnOnly.POST(hmnurl.RegexTicketsAdminScan, adminsOnly(csrfMiddleware(TicketsAdminScanSubmit)))
	...
	hmnOnly.GET(hmnurl.RegexTicketSingleForEvent, needsAuth(TicketSingleForEvent))
	...
	hmnOnly.POST(hmnurl.RegexLoginAction, securityTimerMiddleware(time.Millisecond*100, LoginAction))
	...
	hmnOnly.POST(hmnurl.RegexDiscordBotDebugToggleRecordAll, adminsOnly(DiscordBotDebugToggleRecordAll))
