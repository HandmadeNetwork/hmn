...
func LoginPage(c *RequestContext) ResponseData {
	...
	res.MustWriteTemplate("auth_login.html", LoginPageData{
		BaseData:            getBaseTemplateData(c, "Log in", nil),
...
func RegisterNewUser(c *RequestContext) ResponseData {
	...
	tmpl := RegisterPageData{
		BaseData:            getBaseTemplateData(c, "Register", nil),
		DestinationURL:      dest,
		Notice:              notice,
...
func RegisterNewUserSubmit(c *RequestContext) ResponseData {
	...
	{
		b := c.Perf.StartBlock("SQL", "Check blacklist")
		defer b.End()

		blacklisted, err := isBlacklisted(c, c.Conn, username, emailAddress)
		if err != nil {
	...
	{
		b := c.Perf.StartBlock("AUTH", "Check for existing usernames and emails")
		defer b.End()

		_, err := db.QueryOneScalar[int](c, c.Conn,
			`
			---- Load users with same username
			SELECT id
			FROM hmn_user
			WHERE LOWER(username) = LOWER($1)
			`,
	...
	var ott string
	{
		b := c.Perf.StartBlock("AUTH", "Create user and one time token")
		defer b.End()
...
func EmailConfirmationSubmit(c *RequestContext) ResponseData {
	...
	{
		b := c.Perf.StartBlock("SQL", "Updating user status and deleting token")
		defer b.End()
...
func RequestPasswordReset(c *RequestContext) ResponseData {
	...
	var res ResponseData
	res.MustWriteTemplate("auth_password_reset.html", getBaseTemplateData(c, "Password Reset", nil), c.Perf)
	return res
}
...
func PasswordResetSent(c *RequestContext) ResponseData {
	...
	res.MustWriteTemplate("auth_password_reset_sent.html", PasswordResetSentData{
		BaseData:     getBaseTemplateData(c, "Password Reset", nil),
		ContactUsUrl: hmnurl.BuildContactPage(),
	}, c.Perf)
	return res
}
...
func tryLogin(c *RequestContext, user *models.User, password string) (bool, error) {
	...
	defer c.Perf.StartBlock("AUTH", "Checking password").End()
	...
	if hashed.IsOutdated() {
		b := c.Perf.StartBlock("AUTH", "Re-hash password")
		defer b.End()
...
func loginUser(c *RequestContext, user *models.User, responseData *ResponseData) error {
	defer c.Perf.StartBlock("AUTH", "Setting last login and creating session").End()
