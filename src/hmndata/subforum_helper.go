...
func (node *SubforumTreeNode) GetLineage() []*models.Subforum {
	current := node
	length := 0
	for current != nil {
	...
	result := make([]*models.Subforum, length)
	current = node
...
func GetFullSubforumTree(ctx context.Context, conn db.ConnOrTx) SubforumTree {
	subforums, err := db.Query[models.Subforum](ctx, conn,
		`
		---- Get subforum tree
		SELECT $columns
		FROM subforum
...
type SubforumLineageBuilder struct {
	Tree          SubforumTree
	SubforumCache map[int][]*models.Subforum
...
func MakeSubforumLineageBuilder(fullSubforumTree SubforumTree) *SubforumLineageBuilder {
	return &SubforumLineageBuilder{
		...
		SubforumCache: make(map[int][]*models.Subforum),
		SlugCache:     make(map[int][]string),
	}
}

func (cl *SubforumLineageBuilder) GetLineage(sfId int) []*models.Subforum {
...
func (cl *SubforumLineageBuilder) GetSubforumLineage(sfId int) []*models.Subforum {
